...
| [AWS-VPC-Lattice](https://github.com/elveli/AWS-VPC-Lattice) | An interactive React simulator for AWS VPC Lattice — multi-account/multi-VPC service networking, SigV4 IAM authorization, and weighted canary routing — paired with a real, deployable 2-account stack (EC2 + Lambda targets, cross-account RAM sharing) you can actually `curl` against, not just read about. |
